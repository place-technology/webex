    ...
    def request(method : String, url : String, **kwargs) : Halite::Response
      ...
      loop do
        ...
        begin
          ...
          raise Exceptions::StatusCode.new(status_code.message, status_code.code) if !status_code.valid?
          ...
          raise exception
        rescue exception : Exceptions::RateLimit
          Log.error(exception: exception) { "Requests to the API are being rate limited" }
